...
const columns = [
  ...
  },
  {
    ...
    render: (idtable) => (
      <Link
        to={`${appPath.authority.root}${appPath.authority.approvesubjectsix}/${idtable}`}
      >
        รายละเอียด
      </Link>
    ),
...
const ForAuthTableSubjectSix = () => {
  ...
  const getDocument = async () => {
    try {
      const res = await DocumentSixService.getTablesSixForAuthority();
      setData(res);
      console.log(res);
    } catch (error) {}
  };
...
export default ForAuthTableSubjectSix;
