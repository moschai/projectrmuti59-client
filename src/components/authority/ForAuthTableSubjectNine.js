...
const columns = [
  ...
  },
  {
    ...
    render: (idtable) => (
      <Link
        to={`${appPath.authority.root}${appPath.authority.approvesubjectnine}/${idtable}`}
      >
        รายละเอียด
      </Link>
    ),
...
const ForAuthTableSubjectNine = () => {
  ...
  const getDocument = async () => {
    try {
      const res = await DocumentNineService.getTablesNineForAuthority();
      setData(res);
      console.log(res);
    } catch (error) {}
  };
...
export default ForAuthTableSubjectNine;
