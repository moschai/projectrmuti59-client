...
const columns = [
  ...
  },
  {
    ...
    render: (idtable) => (
      <Link
        to={`${appPath.authority.root}${appPath.authority.approvesubjectten}/${idtable}`}
      >
        รายละเอียด
      </Link>
    ),
...
const ForAuthTableSubjectTen = () => {
  ...
  const getDocument = async () => {
    try {
      const res = await DocumentTenService.getTablesTenForAuthority();
      setData(res);
      console.log(res);
    } catch (error) {}
  };
...
export default ForAuthTableSubjectTen;
