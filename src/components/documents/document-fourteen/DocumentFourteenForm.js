...
const { Option } = Select;
const DocumentFourteenForm = ({ title }) => {
  ...
  const createDocumentFourteen = async (values) => {
    setCreating(true);
    try {
      const documentFourteenResponse = await DocumentFourteenService.createDocumentFourteen(
        values
      );
      form.resetFields();
      Modal.success({
        ...
        content: (
          <span>
            รหัสสำหรับติดตามแบบคำร้องคือ {documentFourteenResponse.id}
          </span>
        ),
        cancelText: false,
      });

      console.log(documentFourteenResponse);
    } catch (error) {
      console.error(error);
  ...
  if (isLoading) {
    ...
    return (
      ...
      >
        <h2 className="text-center">แบบคำร้องขอใบรับรองความประพฤติ</h2>
        ...
        <Col xs={24} sm={24} md={12} span={12}>
          <Form.Item
            label="มีความประสงค์ขอใบรับรองความประพฤติ เพื่อนำไปใช้(โปรดระบุเหตุผล)"
            name="behavioralreceipt"
            rules={[
...
export default DocumentFourteenForm;
