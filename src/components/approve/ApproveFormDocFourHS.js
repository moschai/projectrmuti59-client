...
const ApproveFormDocFourHs = ({
  ...
  const ref = useRef();
  const [showotherapprove, setShowotherapprove] = useState(false);
  const [form] = useForm();
  ...
  return (
    ...
    >
      <Form onFinish={onFinish} form={form} id="approveForm">
        <Form.Item name="selectcondition" label="ความคิดเห็น">
          <Radio.Group>
            <Radio value="1">นักศึกษาลงทะเบียนเรียนต่ำกว่าเกณฑ์</Radio>
            <Radio value="2">นักศึกษาลงทะเบียนเกินกว่าเกณฑ์เป็นครั้งแรก</Radio>
            <Radio value="3">
              นักศึกษาเคยลงทะเบียนเกินกว่าเกณฑ์แล้ว
              <br />
        ...
        <div style={{ paddingLeft: 10 }}>
          <Form.Item name="otherapprove" label="(ในกรณีอื่นๆ)">
            <Checkbox
              onChange={(e) => {
                setShowotherapprove(e.target.checked);
              }}
            />
          </Form.Item>
...
export default ApproveFormDocFourHs;
